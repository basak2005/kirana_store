...
class DailyReport(models.Model):
    ...
    total_purchases = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0'))
    ...
    net_profit = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0'))
    total_credit_sales = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0'))
    cash_in_hand = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0'))
    ...
    def generate_report(cls, date):
        ...
        try:
            ...
            sales_data = Sale.objects.filter(date__date=date).aggregate(
                ...
                credit_sales=Sum('total_amount', filter=Q(is_credit=True))
            )
            
            total_sales = sales_data['total_sales'] or Decimal('0')
            credit_sales = sales_data['credit_sales'] or Decimal('0')
            ...
            total_purchases = purchases_data['total_purchases'] or Decimal('0')
            ...
            print(f"Error generating daily report for {date}: {str(e)}")
            return None
...
class MonthlyReport(models.Model):
    ...
    total_purchases = models.DecimalField(max_digits=15, decimal_places=2, default=Decimal('0'))
    ...
    average_daily_sales = models.DecimalField(max_digits=12, decimal_places=2, default=Decimal('0'))
    ...
    def generate_report(cls, year, month):
        ...
        try:
            ...
            avg_daily_sales = (totals['total_sales'] or Decimal('0')) / days_count if days_count > 0 else Decimal('0')
            ...
            # Log error or handle as needed
            print(f"Error generating monthly report for {month}/{year}: {str(e)}")
            return None
